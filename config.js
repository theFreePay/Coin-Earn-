...
let appState = {
  ...
  lastDailyClaimDate: null, // TODO (کاربر): از ستون last_daily_claim_date سوپابیس لود می‌شه
  joinTimestamp: Date.now() - 1000 * 60 * 40, // فرض: ۴۰ دقیقه پیش وارد شده (برای آنلاک بسته‌های زمان‌دار)
  // چرخ شانس شناور: کول‌داونش الان بر پایه‌ی تاریخ UTC (lastFreeWheelSpinDateUTC پایین‌تره) نه تایم‌استمپه
  ...
  // تسک «از ربات ما دیدن کنید» (l1) — روزی فقط یه‌بار قابل انجامه؛ از روی همین
  // تاریخ UTC چک می‌شه (نه یه فلگ done ثابت که با رفرش صفحه از بین می‌رفت).
  lastBotVisitTaskDateUTC: null,
  lang: NEW_USER_LANG,
  ...
  referralBalance: 2,
  // آیا کاربر تا الان حداقل یک خرید واقعی از فروشگاه (ارتقای افکت گردونه یا گیفت) انجام داده؛
  // هم برای باز شدن بسته‌ی سکه‌ی «اولین خرید» استفاده می‌شه، هم برای تسک محدود «اولین خرید از فروشگاه».
  // TODO (کاربر): این باید از سوپابیس لود/سیو بشه.
  hasFirstPurchase: false,
  // شمارش تبلیغ‌های دیده‌شده‌ی «امروز» برای تسک «۵ تبلیغ امروز ببین» — هر روز (بر پایه‌ی
  // تاریخ UTC) خودکار صفر می‌شه. جدا از appState.adsWatched که مجموع کل (برای XP) هست.
  // TODO (کاربر): این دو فیلد باید از سوپابیس لود/سیو بشن.
  adsWatchedToday: 0,
  adsWatchedTodayDate: null,
...
// Adsgram — Block ID واقعی دوم، نوع Rewarded Video (تنها جایگاه Adsgram باقی‌مونده داخل لیست تسک‌های تبلیغاتی؛
// کد قبلی 38896 منقضی شده بود و کلاً حذف شد)
// کول‌داونش رو رو ۲۰ ثانیه گذاشتیم (درخواست کاربر) تا هر بار بعد از دیدنش، ۲۰ ثانیه بعد دوباره باز/قابل‌نمایش بشه.
const ADSGRAM_REWARDED2_CONFIG = { blockId: "39768", reward: 15, cooldownSec: 20 };

// Adsgram — Block ID واقعی سوم، نوع Rewarded Video — مخصوص «چرخ شانس رایگان روزانه»
// و دکمه‌ی «x2» پاداش روزانه (هر دو از همین یک کد تبلیغاتی استفاده می‌کنن).
const ADSGRAM_DAILY_CONFIG = { blockId: "39769" };

// Adsgram — بلاک‌های Task (ویجت <adsgram-task>) — همه‌شون درست بعد از CPX Research
// نشون داده می‌شن. توجه: بلاک تایپ Task باید دقیقاً با پیشوند "task-" باشه.
// TODO (کاربر): پاداش دو بلاک جدید (40701 و 40700) رو فعلاً همون ۲۰ سکه‌ی بلاک قبلی
// گذاشتم — اگه عدد دیگه‌ای مدنظرته همینجا عوضش کن.
const ADSGRAM_TASK_CONFIG = { blockId: "task-39639", reward: 20 };
const ADSGRAM_TASK2_CONFIG = { blockId: "task-40701", reward: 20 };
const ADSGRAM_TASK3_CONFIG = { blockId: "task-40700", reward: 20 };
const ADSGRAM_TASK_WIDGETS = [
  { cardId: "adsgram-task-card-1", elId: "adsgram-task-el-1", config: ADSGRAM_TASK_CONFIG },
  { cardId: "adsgram-task-card-2", elId: "adsgram-task-el-2", config: ADSGRAM_TASK2_CONFIG },
  { cardId: "adsgram-task-card-3", elId: "adsgram-task-el-3", config: ADSGRAM_TASK3_CONFIG },
];
// اگه یه بلاک تسک موقتاً چیزی نداشت (onBannerNotFound)، جاش رو با دیدن همین
// ویدیوی ریواردد پر می‌کنیم تا تا دفعه‌ی بعد که دوباره امتحان می‌کنیم خالی نمونه.
const ADSGRAM_TASK_FALLBACK_RETRY_MS = 8000; // هر ۸ ثانیه دوباره امتحان کن
...
// ----------------------------------------------------------------------------
const AD_NETWORKS = [
  { id: "adsgram2", name: "Adsgram", reward: ADSGRAM_REWARDED2_CONFIG.reward, cooldownSec: ADSGRAM_REWARDED2_CONFIG.cooldownSec, blockId: ADSGRAM_REWARDED2_CONFIG.blockId },
...
// جوایز چرخ شانس شناور — هر ۱۲ ساعت فقط یک‌بار، با دیدن تبلیغ.
// بازه‌ی جایزه: ۸۰۰ تا ۳۰۰۰ سکه (اعداد کوچیک‌تر شانس بیشتری دارن).
const WHEEL_SEGMENTS = [
  { label: "800", coins: 800, weight: 25, color: "#7b3ff2" },
  { label: "1000", coins: 1000, weight: 22, color: "#e04fd6" },
  { label: "1200", coins: 1200, weight: 18, color: "#7b3ff2" },
  { label: "1500", coins: 1500, weight: 14, color: "#e04fd6" },
  { label: "1800", coins: 1800, weight: 10, color: "#7b3ff2" },
  { label: "2200", coins: 2200, weight: 6, color: "#e04fd6" },
  { label: "2600", coins: 2600, weight: 3, color: "#ffcb4d" },
  { label: "3000", coins: 3000, weight: 2, color: "#ffcb4d" },
];
...
// ----------------------------------------------------------------------------
// پاداش‌های هفتگی پاداش روزانه (۷ روزه، بعد از روز ۷ دوباره از روز ۱ شروع می‌شه).
// دو نوع پاداش داریم:
//  - { type: "coins", amount } → سکه‌ی معمولی (دکمه‌ی x2 هم داره)
//  - { type: "coinRainUpgrade" } → روز ۱: یک پله ارتقای افکت «باران سکه» (بدون x2)
//  - { type: "wheelColorUpgrade", steps } → روز ۴: چند پله ارتقای «مسیر رنگ گردونه» (بدون x2)
// اگه کاربر از قبل به سقفِ اون ارتقا رسیده باشه، به‌جاش همون مقدار fallback سکه می‌گیره.
// ----------------------------------------------------------------------------
const DAILY_DAY1_FALLBACK_COINS = 3000; // اگه Coin Rain از قبل تایر ۴ (ماکس) بود
const DAILY_DAY4_FALLBACK_COINS = 8000; // اگه مسیر رنگ گردونه از قبل پله ۸۰ (ماکس) بود
const DAILY_DAY4_WHEEL_STEPS = 3;

const DAILY_REWARDS = [
  { type: "coinRainUpgrade" },                 // روز ۱
  { type: "coins", amount: 5000 },              // روز ۲
  { type: "coins", amount: 10000 },             // روز ۳
  { type: "wheelColorUpgrade", steps: DAILY_DAY4_WHEEL_STEPS }, // روز ۴
  { type: "coins", amount: 4500 },              // روز ۵
  { type: "coins", amount: 2000 },              // روز ۶
  { type: "coins", amount: 10000 },             // روز ۷
];

// تسک‌های محدود (limited) — name به‌صورت چندزبانه
const LIMITED_TASKS = [
  { id: "l1", name: { fa: "به بات اصلی ما سر بزنید", en: "Visit our main bot", ru: "Загляните в наш основной бот" }, reward: 40, done: false },
...
// ----------------------------------------------------------------------------
// سیستم رفرال واقعی (جدول referrals توی Supabase).
// لینک دعوت: {REFERRAL_BOT_LINK_BASE}{telegram_id خودت}
// وقتی دوستت با این لینک وارد میشه (startapp=<آیدی تو>)، دو تا پاداش فوری و
// یک‌باره از طریق RPC «register_referral_join» داده می‌شه:
//   - به خودِ دوستت (رفرال‌شده): REFERRAL_JOIN_BONUS_REFERRED سکه
//   - به تو (دعوت‌کننده): REFERRAL_JOIN_BONUS_REFERRER سکه (بدون اینکه هنوز عدد
//     «رفرال اکتیو»ت بالا بره)
// بعدش، وقتی همون دوستت ۵ تا تسک «تبلیغ ویژه» (Tads.me) رو کامل کنه، از طریق RPC
// «increment_referral_task» عدد رفرال اکتیو (users.referral_balance) خودت +۱
// می‌شه — همونی که برای خرید گیفت‌های تلگرام لازمه.
// TODO (کاربر): این دو RPC باید توی Supabase (SQL editor) ساخته بشن؛ فایل SQL
// جداگونه براش آماده شده.
// ----------------------------------------------------------------------------
const REFERRAL_BOT_LINK_BASE = "https://t.me/coin_earn_2026bot/luckyroom?startapp=";
// لینک بات اصلی (بدون میدل‌اپ/استارت‌اپ) — برای تسک «به بات اصلی ما سر بزنید».
// TODO (کاربر): اگه یوزرنیم بات فرق داره، همین رو با آدرس درست عوض کن.
const MAIN_BOT_LINK = "https://t.me/who_story_bot?startapp=true";
const REFERRAL_JOIN_BONUS_REFERRED = 1000;
const REFERRAL_JOIN_BONUS_REFERRER = 500;
const REFERRAL_TASKS_REQUIRED = 5;

// چرخ شانس شناور حالا یک‌بار در روزِ UTC مجازه (نه ۱۲ساعته) — چون بدون تغییر
// دیتابیس، فقط ستون last_free_wheel_spin_date (تاریخ) در دسترسمونه، نه ساعت دقیق.
...
const TRANSLATIONS = {
  fa: {
    ...
    referralHint: "دوستت با ورود ۵۰۰ سکه بهت می‌ده؛ وقتی هم ۵ تا «تبلیغ ویژه» رو کامل کنه، رفرالت اکتیو می‌شه",
    inviteFriends: "دعوت دوستان 🚀",
    inviteMessage: "من دارم این بازی رو انجام می‌دم و واقعاً جالبه، به توهم پیشنهادش می‌کنم 🎮\nبا لینک من وارد شو و سکه‌ی رایگان بگیر 👇",
    ...
    cpxPending: "بعد از تکمیل واقعی نظرسنجی، سکه‌ت خودکار اضافه می‌شه ⏳",
  ...
  en: {
    ...
    referralHint: "You get coins the moment your friend joins; once they finish 5 \"Special offer\" tasks, the referral becomes active",
    inviteFriends: "Invite friends 🚀",
    inviteMessage: "I'm playing this game and it's actually fun, thought you'd like it too 🎮\nJoin with my link and get free coins 👇",
    ...
    cpxPending: "Coins will be added automatically once the survey is actually completed ⏳",
  ...
  ru: {
    ...
    referralHint: "Ты получишь монеты сразу, как друг зайдёт; когда он выполнит 5 «специальных» заданий, реферал станет активным",
    inviteFriends: "Пригласить друзей 🚀",
    inviteMessage: "Я играю в эту игру, реально интересно, тебе тоже советую 🎮\nЗаходи по моей ссылке и получи бесплатные монеты 👇",
    ...
    cpxPending: "Монеты начислятся автоматически после реального завершения опроса ⏳",
