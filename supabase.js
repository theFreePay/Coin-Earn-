...
function applyUserRowToAppState(row) {
  ...
  appState.lastBotVisitTaskDateUTC = row.last_bot_visit_task_date || null;
}
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    ...
    // کاربر برگشت به اپ (مثلاً بعد از تکمیل یه سوروی CPX توی مرورگر بیرونی) —
    // موجودی رو با دیتابیس چک کن، شاید Postback زودتر رسیده باشه.
  ...
  try {
    const { error } = await supabaseClient
      ...
      .update({
        ...
        last_bot_visit_task_date: appState.lastBotVisitTaskDateUTC,
...
// -----------------------------------------------------------------------------
// خوندن مستقیم موجودی از دیتابیس — برای تشخیص واریزهایی که بیرون از این تب/سشن
// اتفاق افتادن (مثل CPX که سکه‌ی واقعی رو از طریق Postback خودِ سرور به سرور،
// مستقیم توی دیتابیس واریز می‌کنه، نه از طریق کلاینت).
// -----------------------------------------------------------------------------
async function fetchLatestCoinsFromServer() {
  ...
  try {
    ...
    console.log("⚠️ خطا در fetchLatestCoinsFromServer:", err.message || err);
    return null;
  }
}

// appState.coins رو با دیتابیس هماهنگ می‌کنه. اگه موجودی سرور از موجودی محلی
// بیشتر بود (یعنی یه واریز واقعی بیرون از این تب انجام شده)، UI رو آپدیت می‌کنه
// و مقدار اضافه‌شده رو برمی‌گردونه (برای toast/آمار). اگه کمتر/مساوی بود، دست نمی‌زنه
// (چون ممکنه یه sync محلیِ در حال ارسال هنوز به دیتابیس نرسیده باشه).
async function refreshCoinsFromServer() {
  const latest = await fetchLatestCoinsFromServer();
  if (latest == null) return 0;
  const diff = latest - appState.coins;
  if (diff > 0) {
    appState.coins = latest;
    const bal = $("#coin-balance");
    if (bal) bal.textContent = fmt(appState.coins);
    const statCoins = $("#stat-coins");
    if (statCoins) statCoins.textContent = fmt(appState.coins);
  }
  return diff;
}
