...
function applyLanguage() {
  ...
  renderNormalTasks();
  renderWheelSkinsGrid(); renderLevelWheelCard();
  if (activeShopCategory === "customize") renderCustomizeGrid();
  renderPlayers();
...
// Shop
// ---------------------------------------------------------------------------
let activeShopCategory = "wheel_skins";
$$("#shop-tabs .task-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    $$("#shop-tabs .task-tab").forEach((t) => t.classList.remove("active"));
    tab.classList.add("active");
    activeShopCategory = tab.dataset.cat;
    if (activeShopCategory === "customize") {
      $("#wheel-skins-grid").style.display = "none";
      $("#customize-grid").style.display = "grid";
      renderCustomizeGrid();
    } else {
      $("#wheel-skins-grid").style.display = "grid";
      $("#customize-grid").style.display = "none";
      renderWheelSkinsGrid(); renderLevelWheelCard();
    }
    sound.click();
  });
...
// ---------------------------------------------------------------------------
// Profile customization (تزئینات — فقط ظاهری، هیچ ارزش مالی/قابل‌فروشی نداره)
// ---------------------------------------------------------------------------
const CUSTOMIZE_STATE_KEY = { frame: "profileFrame", shadow: "profileShadow", font: "profileFont" };

function renderCustomizeGrid() {
  const grid = $("#customize-grid");
  if (!grid) return;
  grid.innerHTML = "";
  CUSTOMIZATION_ITEMS.forEach((item) => {
    const stateKey = CUSTOMIZE_STATE_KEY[item.type];
    const owned = appState[`owned_${item.id}`] === true;
    const applied = appState[stateKey] === item.id;
    const affordable = appState.coins >= item.cost;
    const card = document.createElement("div");
    card.className = "item-card";
    const swatch = item.type === "font"
      ? `<div class="customize-swatch font-preview" style="font-weight:${item.preview}">Aa</div>`
      : item.type === "shadow"
        ? `<div class="customize-swatch" style="background:var(--surface-2); box-shadow:${item.preview}"></div>`
        : `<div class="customize-swatch" style="background:${item.preview}"></div>`;
    // به‌جای «سکه کافی نیست»، قیمت لازم رو خاکستری نشون بده؛ دکمه هم فعال می‌مونه تا کلیکش کاربر رو راهنمایی کنه
    let btnLabel = applied ? tr("appliedBtn") : owned ? tr("applyBtn") : affordable ? `${tr("buyBtn")} · ${fmt(item.cost)}` : `<span class="price-greyed">${fmt(item.cost)}</span>`;
    card.innerHTML = `
      ${swatch}
      <div class="item-name">${trField(item.name)}</div>
      <button class="item-btn${!applied && !owned && !affordable ? " btn-need-more" : ""}" ${applied ? "disabled" : ""}>${btnLabel}</button>
    `;
    if (!applied) {
      card.querySelector("button").addEventListener("click", () => {
        if (!owned && !affordable) {
        ...
        if (!owned) {
          ...
          appState[`owned_${item.id}`] = true;
          $("#coin-balance").textContent = fmt(appState.coins);
          sound.coin();
        }
        appState[stateKey] = item.id;
        applyProfileCustomization();
        showToast(`✅ ${trField(item.name)}`);
        haptic("medium");
        renderCustomizeGrid();
      });
    }
...
const FRAME_CLASS_MAP = { frame_gold: "frame-gold", frame_neon: "frame-neon", frame_ice: "frame-ice" };
const SHADOW_CLASS_MAP = { shadow_glow: "shadow-glow", shadow_soft: "shadow-soft" };

function applyProfileCustomization() {
  const ring = $("#avatar-ring");
  if (!ring) return;
  Object.values(FRAME_CLASS_MAP).forEach((c) => ring.classList.remove(c));
  Object.values(SHADOW_CLASS_MAP).forEach((c) => ring.classList.remove(c));
  // نکته: اگه عکس واقعی تلگرام روی avatar-ring ست شده باشه (background-image: url(...))
  // کلاس فریم فقط رنگ حاشیه رو عوض می‌کنه، عکس دست‌نخورده می‌مونه.
  if (FRAME_CLASS_MAP[appState.profileFrame]) ring.classList.add(FRAME_CLASS_MAP[appState.profileFrame]);
  if (SHADOW_CLASS_MAP[appState.profileShadow]) ring.classList.add(SHADOW_CLASS_MAP[appState.profileShadow]);
  const font = CUSTOMIZATION_ITEMS.find((i) => i.id === appState.profileFont);
  const nameEl = $("#profile-name");
  if (nameEl) nameEl.style.fontWeight = font ? font.preview : "";
}
  ...
  REFERRALS.forEach((ref) => {
    ...
    if (ref.status === "claimable") {
      ...
      row.querySelector(".referral-status").addEventListener("click", () => {
        const boosted = isReferralBoostActive();
        const reward = boosted ? REFERRAL_BOOST_REWARD : ref.reward;
        addCoins(reward);
        showToast(`+${reward}! 🎉`);
...
// ---------------------------------------------------------------------------
// Referral boost banner (۳ ساعت اول) + مودال فوریت با ساعت شنی
// ---------------------------------------------------------------------------
function isReferralBoostActive() {
  return Date.now() - appState.joinTimestamp < REFERRAL_BOOST_WINDOW_MS;
}
function updateBoostBanner() {
  const banner = $("#boost-banner");
  const remaining = REFERRAL_BOOST_WINDOW_MS - (Date.now() - appState.joinTimestamp);
  if (remaining <= 0) {
    banner.classList.add("hidden");
    return;
  }
  banner.classList.remove("hidden");
  $("#boost-multiplier").textContent = `x${REFERRAL_BOOST_MULTIPLIER}`;
  $("#boost-label").textContent = tr("boostActiveText");
  const timeStr = formatHMS(remaining);
  $("#boost-timer").textContent = timeStr;
  if (!$("#boost-modal-backdrop").classList.contains("hidden")) {
    $("#boost-modal-timer").textContent = timeStr;
  }
}
...
setInterval(updateBoostBanner, 1000);

$("#boost-banner").addEventListener("click", () => {
  if (!isReferralBoostActive()) return;
  $("#boost-modal-backdrop").classList.remove("hidden");
  $("#boost-modal-timer").textContent = formatHMS(REFERRAL_BOOST_WINDOW_MS - (Date.now() - appState.joinTimestamp));
  sound.click();
  haptic("light");
});
$("#boost-modal-invite-btn").addEventListener("click", () => {
  const text = $("#referral-link-text").textContent;
  navigator.clipboard?.writeText(text).catch(() => {});
  showToast(tr("copied"));
  $("#boost-modal-backdrop").classList.add("hidden");
  sound.click();
  haptic("medium");
});
// بستن مودال بوست با کلیک روی بک‌دراپ (بیرون کارت)
$("#boost-modal-backdrop").addEventListener("click", (e) => {
  if (e.target.id === "boost-modal-backdrop") $("#boost-modal-backdrop").classList.add("hidden");
});
