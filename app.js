...
function renderAdTasks() {
  ...
  AD_NETWORKS.forEach((net) => {
    ...
    const slotHtml = net.id === "tads" ? `<div id="tads-container-${TADS_CONFIG.widgetId}" class="tads-ad-slot"></div>` : "";
    card.innerHTML = `
      <div class="task-left">
...
function watchAdTask(networkId) {
  ...
  if (networkId === "tads") {
    ...
    const adController = window.tads.init({
      ...
      type: "static",
      debug: false,
      onClickReward: () => onAdSuccess(),
      onAdsNotFound: () => {
        showToast(tr("noAdsFound"));
        btn.disabled = false;
        btn.textContent = tr("watchAd");
      },
    });
    adController.loadAd().then(() => {
      ...
      btn.style.display = "none"; // پاداش با کلیک خودِ کاربر روی تبلیغ داده میشه، نه این دکمه
    }).catch((err) => {
      console.log("TADS load failed:", err);
      btn.disabled = false;
      btn.textContent = tr("watchAd");
    });
    return;
