...
function goToPage(name) {
  ...
  if (name === "tasks") { refreshAdsWatchedTodayFromDate(); renderNormalTasks(); renderLimitedTasks(); }
  sound.click();
  haptic("light");
}
...
$("#spin-btn").addEventListener("click", () => {
  ...
  const btn = $("#spin-btn");
  btn.disabled = true;
  showAdsgramRewardedAd(
    ADSGRAM_DAILY_CONFIG.blockId,
    () => spinFreeWheel(),
    () => { if (!hasSpunFreeWheelToday()) btn.disabled = false; }
  );
});
...
function spinPotWheel() {
  ...
  animateWheelRotation(
    ...
    () => {
      ...
      // به محض این‌که نتیجه‌ی دور مشخص شد (چه برد چه باخت)، فوراً و بدون صبر برای
      // debounce معمولی، موجودی نهایی سکه رو توی دیتابیس سیو کن — تا اگه کاربر
      // همین لحظه (مثلاً درست بعد از باخت) از اپ خارج بشه، کسر سهمش از دست نره.
      flushUserSyncNow();
...
// اگه از آخرین باری که تبلیغ دیده شده روز (UTC) عوض شده باشه، شمارنده‌ی «تبلیغ امروز» صفر می‌شه.
function refreshAdsWatchedTodayFromDate() {
  const today = getTodayUTCDateStr();
  if (appState.adsWatchedTodayDate !== today) {
    appState.adsWatchedTodayDate = today;
    appState.adsWatchedToday = 0;
  }
}

// باید بعد از هر تبلیغ موفق (هر شبکه‌ای) صدا زده بشه تا تسک «۵ تبلیغ امروز ببین» آپدیت بشه.
function incrementAdsWatchedToday() {
  refreshAdsWatchedTodayFromDate();
  appState.adsWatchedToday = (appState.adsWatchedToday || 0) + 1;
  renderNormalTasks();
}
...
function renderDailyActions() {
  ...
  if (!appState.dailyClaimedToday) {
    if (reward.type === "coins") {
      ...
      $("#claim-x2-btn").addEventListener("click", (e) => {
...
// یه تبلیغ واقعی OnClicka (adclicka) نشون می‌ده. فقط بعد از تایید واقعی دیده‌شدن تبلیغ
// (resolve شدن پرامیس) onSuccess صدا زده می‌شه؛ اگه رد بشه/بسته بشه یا هنوز آماده نباشه، onCancel.
function showOnclickaRewardedAd(onSuccess, onCancel) {
  if (!onclickaShowAd) {
  ...
  onclickaShowAd().then(onSuccess).catch((err) => {
    console.log("Ad failed/skipped:", err);
    if (onCancel) onCancel();
  });
}

// نگه‌داشتن AdController های واقعی Adsgram بعد از init شدن SDK — چون چند تا بلاک آیدی
// رودرن (Rewarded) داریم (تسک‌ها + گردونه/پاداش روزانه)، هرکدوم رو با کلید blockId خودش نگه می‌داریم.
let adsgramControllers = {};
const ADSGRAM_REWARDED_BLOCK_IDS = [
  ADSGRAM_REWARDED2_CONFIG.blockId,
  ADSGRAM_DAILY_CONFIG.blockId,
];
function initAdsgramSdk() {
  ...
  ADSGRAM_REWARDED_BLOCK_IDS.forEach((blockId) => {
    if (!adsgramControllers[blockId]) {
      adsgramControllers[blockId] = window.Adsgram.init({ blockId });
    }
  });
}

// نمایش یه تبلیغ Rewarded Video از Adsgram با یه blockId مشخص (برای جاهایی خارج از
// لیست تسک‌های تبلیغاتی، مثل چرخ شانس رایگان روزانه و دکمه‌ی x2 پاداش روزانه).
function showAdsgramRewardedAd(blockId, onSuccess, onCancel) {
...
function watchAdTask(networkId) {
  ...
  if (networkId === "adsgram2") {
    const controller = adsgramControllers[net.blockId];
    if (!controller) {
      showToast("⏳ در حال آماده‌سازی تبلیغ، یک لحظه صبر کن...");
      btn.disabled = false;
      btn.textContent = tr("watchAd");
    ...
    controller.show().then(onAdSuccess).catch(onAdFailure);
    return;
  }
...
// ---------------------------------------------------------------------------
// Adsgram — Task ad format (وب‌کامپوننت <adsgram-task>). چند بلاک داریم
// (ADSGRAM_TASK_WIDGETS توی config.js)، همه‌ی همین منطق رو هرکدوم مستقل اجرا می‌کنن.
// خودِ ویجت مسئول لود/رندر/نمایش تسکه؛ ما فقط به رویدادهاش گوش می‌دیم:
// - reward: کاربر تسک رو کامل کرد و باید سکه بگیره
// - onBannerNotFound: فعلاً هیچ تسکی برای این بلاک نیست → به‌جاش یه کارت
//   «ویدیوی ریواردد» (همون بلاک ADSGRAM_REWARDED2_CONFIG) نشون می‌دیم و هر
//   چند ثانیه (ADSGRAM_TASK_FALLBACK_RETRY_MS) دوباره خودِ بلاک رو امتحان می‌کنیم؛
//   هروقت چند ثانیه بعد از یه تلاش، onBannerNotFound دوباره fire نشد یعنی این بار
//   بلاک واقعی برگشته و دیگه دست از تلاش دوباره می‌کشیم.
// - onError / onTooLongSession: فقط لاگ/توست
//
// باگ قبلی: یه فلگ سراسری (adsgramTaskRewarded) فقط یه‌بار true می‌شد و هیچ‌وقت
// ریست نمی‌شد؛ در نتیجه دفعه‌ی دومی که همون بلاک یه تسک تازه نشون می‌داد و کاربر
// انجامش می‌داد، دیگه سکه‌ای داده نمی‌شد. الان هر ویجت قفل مخصوص خودشو داره و این
// قفل فقط چند ثانیه (برای فیلتر کردن fire شدن تکراری خودِ همون ایونت) نگه داشته
// می‌شه، نه برای همیشه — پس تسک‌های بعدی همون بلاک هم سکه می‌دن.
// ---------------------------------------------------------------------------
const ADSGRAM_TASK_REWARD_LOCK_MS = 4000;
const ADSGRAM_TASK_SUCCESS_GRACE_MS = 3000;

function buildAdsgramRealTaskCard(widget) {
  const card = document.createElement("div");
  card.className = "task-card hidden-task";
  card.id = widget.cardId;
  card.innerHTML = `
    <adsgram-task class="adsgram-task-el" data-block-id="${widget.config.blockId}" data-debug="false" data-debug-console="false">
      <span slot="reward" class="ag-task-reward"></span>
      <div slot="button" class="ag-task-btn" data-i18n="watchAd">${tr("watchAd")}</div>
      <div slot="claim" class="ag-task-btn ag-task-btn-claim" data-i18n="claim">${tr("claim")}</div>
      <div slot="done" class="ag-task-btn ag-task-btn-done" data-i18n="done">${tr("done")}</div>
    </adsgram-task>
  `;
  return card;
}

function buildAdsgramFallbackTaskCard(widget) {
  const card = document.createElement("div");
  card.className = "task-card ag-fallback-card";
  card.id = widget.cardId;
  card.innerHTML = `
    <div class="task-left">
      <div class="task-icon">Ad</div>
      <div><div class="task-name" data-i18n="watchAd">${tr("watchAd")}</div><div class="task-sub">+${getAdRewardDisplay(ADSGRAM_REWARDED2_CONFIG.reward)}</div></div>
    </div>
    <button class="task-btn">${tr("watchAd")}</button>
  `;
  const btn = card.querySelector(".task-btn");
  btn.addEventListener("click", () => {
    ...
    showAdsgramRewardedAd(
      ADSGRAM_REWARDED2_CONFIG.blockId,
      () => {
        const reward = getAdRewardWithBoost(ADSGRAM_REWARDED2_CONFIG.reward);
        addCoins(reward);
        appState.adsWatched = (appState.adsWatched || 0) + 1;
        incrementAdsWatchedToday();
        refreshLevelFromXp();
        renderHomeLevelCard();
        renderLevelWheelCard();
        showToast(`+${reward} — Adsgram`);
        logAdTransaction("adsgram_task_fallback", widget.config.blockId, reward);
        queueUserSync();
        btn.disabled = false;
        btn.textContent = tr("watchAd");
      },
...
function initAdsgramTaskWidget(widget) {
  let rewardLocked = false;
  let retryTimer = null;
  let successGraceTimer = null;

  function attachRealListeners(card) {
    const el = card.querySelector(".adsgram-task-el");
    if (!el) return;
    const rewardSlot = card.querySelector(".ag-task-reward");
    if (rewardSlot) rewardSlot.textContent = `+${getAdRewardDisplay(widget.config.reward)}`;
    ...
    el.addEventListener("reward", () => {
      ...
      rewardLocked = true;
      setTimeout(() => { rewardLocked = false; }, ADSGRAM_TASK_REWARD_LOCK_MS);

      const reward = getAdRewardWithBoost(widget.config.reward);
      addCoins(reward);
      appState.adsWatched = (appState.adsWatched || 0) + 1;
      incrementAdsWatchedToday();
      refreshLevelFromXp();
      ...
      renderLevelWheelCard();
      showToast(`+${reward} — Adsgram`);
      logAdTransaction("adsgram_task", widget.config.blockId, reward);
      queueUserSync();
    });

    el.addEventListener("onError", (e) => {
      console.log(`Adsgram task (${widget.config.blockId}) error:`, e.detail);
    });
    ...
    el.addEventListener("onTooLongSession", () => {
      showToast("⏳ سشن خیلی طولانی شد، لطفاً اپ رو دوباره باز کن");
    });
  ...
  function swapToFallback() {
    clearTimeout(successGraceTimer);
    const current = document.getElementById(widget.cardId);
    if (current && !current.classList.contains("ag-fallback-card")) {
      current.replaceWith(buildAdsgramFallbackTaskCard(widget));
    }
    if (!retryTimer) {
      retryTimer = setInterval(tryReloadRealWidget, ADSGRAM_TASK_FALLBACK_RETRY_MS);
    }
  }

  function tryReloadRealWidget() {
    const current = document.getElementById(widget.cardId);
    if (!current) return;
    const realCard = buildAdsgramRealTaskCard(widget);
    current.replaceWith(realCard);
    attachRealListeners(realCard);
    // اگه ظرف چند ثانیه onBannerNotFound دوباره fire نشه، یعنی این بار بلاک واقعاً
    // برگشته؛ دیگه لازم نیست هر چند ثانیه یه‌بار دوباره بسازیمش.
    clearTimeout(successGraceTimer);
    successGraceTimer = setTimeout(() => {
      if (retryTimer) { clearInterval(retryTimer); retryTimer = null; }
    }, ADSGRAM_TASK_SUCCESS_GRACE_MS);
  }

  const initialCard = document.getElementById(widget.cardId);
  if (initialCard) attachRealListeners(initialCard);
}

function setupAdsgramTaskWidgets() {
  ADSGRAM_TASK_WIDGETS.forEach(initAdsgramTaskWidget);
}
...
function setTasksNavDotVisible(visible) {
  ...
  const adsTabDot = $("#ads-tab-dot");
  if (adsTabDot) adsTabDot.classList.toggle("show", visible);
}
...
function renderCpxTasks(surveys) {
  ...
  btn.addEventListener("click", () => {
    ...
    // توجه: اینجا دیگه سکه فوری داده نمی‌شه — چون سکه‌ی واقعی رو Edge Function
    // (بعد از تأیید واقعیِ CPX از طریق Postback) مستقیم توی دیتابیس واریز می‌کنه.
    // اگه اینجا هم فوری می‌دادیم، کاربر دوبار پول می‌گرفت.
    logAdTransaction("cpx", s.id, coins); // فقط لاگ/ردیابی، status پیش‌فرضش "pending"ه

    btn.disabled = true;
    btn.classList.add("cooldown");
    btn.textContent = "⏳";
    showToast(tr("cpxPending"));
    card.classList.add("fading");
    setTasksNavDotVisible(false);
...
// -----------------------------------------------------------------------------
// بعد از این‌که کاربر روی تسک CPX کلیک کرد و رفت سوروی رو انجام بده، هر چند ثانیه
// یه‌بار موجودی رو از دیتابیس چک می‌کنیم تا ببینیم Postback واقعی رسیده یا نه.
// اگه رسید (یعنی موجودی سرور از موجودی محلی بیشتر شده)، جشن می‌گیریم و آمار محلی
// (ads_watched/XP) رو هم به‌روز می‌کنیم. اگه بعد از چند دقیقه هیچی نرسید، ولش می‌کنیم —
// چرخه‌ی خودکار fetchAndRenderCpxTasks هرحال دوباره کارت رو تازه می‌کنه.
// -----------------------------------------------------------------------------
let cpxPollTimer = null;
function startCpxRewardPolling() {
  if (cpxPollTimer) clearInterval(cpxPollTimer);
  const POLL_INTERVAL_MS = 15000;
  const MAX_POLLS = 20; // تقریباً ۵ دقیقه
  let pollCount = 0;
  cpxPollTimer = setInterval(async () => {
    pollCount++;
    const diff = await refreshCoinsFromServer();
    if (diff > 0) {
      ...
      renderHomeLevelCard();
      renderLevelWheelCard();
      showToast(`+${fmt(diff)} — CPX ✅`);
      queueUserSync(); // آمار محلی (ads_watched/XP) رو سیو کن؛ خودِ coins که از سرور اومده نیاز به sync نداره
...
function hasDoneBotVisitTaskToday() {
  return appState.lastBotVisitTaskDateUTC === getTodayUTCDateStr();
}
...
function renderLimitedTasks() {
  ...
  LIMITED_TASKS.forEach((task) => {
    // تسک «اولین خرید از فروشگاه» تا وقتی کاربر واقعاً یه خرید (ارتقای افکت گردونه یا گیفت)
    // انجام نداده قفله؛ نیازی به بک‌اند نداره چون از appState.hasFirstPurchase محلی می‌خونه.
    const isShopPurchaseTask = task.id === "l3";
    const isBotVisitTask = task.id === "l1";
    // l1 («از ربات ما دیدن کنید») روزی فقط یه‌بار قابل انجامه (بر پایه‌ی تاریخ UTC،
    // نه یه done همیشگی) — بقیه‌ی تسک‌های محدود (مثل l3) همون رفتار همیشگیِ done دارن.
    const done = isBotVisitTask ? hasDoneBotVisitTaskToday() : task.done;
    const locked = isShopPurchaseTask && !task.done && !appState.hasFirstPurchase;
    const card = document.createElement("div");
    card.className = "task-card";
    let btnLabel;
    if (done) btnLabel = "✓";
    else if (locked) btnLabel = "🔒";
    else btnLabel = tr("getIt");
    card.innerHTML = `
      <div class="task-left">
        <div class="task-icon">★</div>
        <div><div class="task-name">${trField(task.name)}</div><div class="task-sub">+${task.reward}</div></div>
      </div>
      <button class="task-btn" ${done || locked ? "disabled" : ""}>${btnLabel}</button>
    `;
    if (!done && !locked) {
      card.querySelector("button").addEventListener("click", () => {
        if (task.id === "l1") {
          sound.click();
          const tg = window.Telegram && window.Telegram.WebApp;
          if (tg && tg.openTelegramLink) tg.openTelegramLink(MAIN_BOT_LINK);
          else if (tg && tg.openLink) tg.openLink(MAIN_BOT_LINK);
          else window.open(MAIN_BOT_LINK, "_blank");
          appState.lastBotVisitTaskDateUTC = getTodayUTCDateStr();
        } else {
          task.done = true;
        }
...
// پیشرفت واقعی هر تسک معمولی رو از روی داده‌ی واقعی appState/cachedReferrals برمی‌گردونه
// (بدون نیاز به بک‌اند اضافه — همه از منابعی‌ان که از قبل توی اپ وجود دارن).
function getNormalTaskProgress(task) {
  if (task.id === "n1") return Math.min(cachedReferrals.length, task.total); // ۳ دوست دعوت کن — رفرال‌های واقعی
  if (task.id === "n2") return Math.min(appState.adsWatchedToday || 0, task.total); // ۵ تبلیغ امروز
  if (task.id === "n3") return Math.min(appState.dailyStreak || 0, task.total); // ۷ روز پشت‌سرهم — استریک واقعی پاداش روزانه
  return Math.min(task.progress || 0, task.total);
}
...
function renderNormalTasks() {
  ...
  NORMAL_TASKS.forEach((task) => {
    const progress = getNormalTaskProgress(task);
    const complete = progress >= task.total;
    const card = document.createElement("div");
    card.className = "task-card";
    card.innerHTML = `
      <div class="task-left">
        <div class="task-icon">${progress}/${task.total}</div>
        <div><div class="task-name">${trField(task.name)}</div><div class="task-sub">+${task.reward}</div></div>
      </div>
...
// دکمه‌ی «دعوت دوستان»: صفحه‌ی اشتراک‌گذاری/فوروارد خودِ تلگرام رو با یه پیام
// آماده (متن دعوت + لینک رفرال) باز می‌کنه؛ کاربر فقط چت مقصد رو انتخاب می‌کنه.
$("#invite-friends-btn").addEventListener("click", () => {
  const link = $("#referral-link-text").textContent;
  const shareUrl = `https://t.me/share/url?url=${encodeURIComponent(link)}&text=${encodeURIComponent(tr("inviteMessage"))}`;
  const tg = window.Telegram && window.Telegram.WebApp;
...
async function boot() {
 try {
  ...
  // هر خطای غیرمنتظره‌ای هم که بیفته، splash نباید کاربر رو برای همیشه گیر بندازه
  console.log("⚠️ خطا در boot():", err && err.message || err);
 } finally {
  // اطلاعات کاربر (چه کاربر قدیمی، چه تازه) الان کامل لود/ساخته شده و رندر اولیه
  // انجام شده — پس splash می‌تونه بره کنار. برای کاربر تازه، پنجره‌ی انتخاب زبون
  // بالاتر همین الان باز شده، پس همون لحظه که splash محو می‌شه، مودال زیرش آماده‌ست.
