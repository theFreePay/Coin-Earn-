...
// ---------------------------------------------------------------------------
// Adsgram — Task ad format (وب‌کامپوننت <adsgram-task>، blockId: "task-39639").
// خودِ ویجت مسئول لود/رندر/نمایش تسکه؛ ما فقط به رویدادهاش گوش می‌دیم:
// - reward: کاربر تسک رو کامل کرد و باید سکه بگیره
// - onBannerNotFound: هیچ تسکی برای نمایش نیست → کارت رو مخفی می‌کنیم
// - onError / onTooLongSession: فقط لاگ/توست
// ---------------------------------------------------------------------------
let adsgramTaskRewarded = false; // جلوگیری از دو بار سکه دادن اگه ایونت reward دوبار fire بشه
function setupAdsgramTaskWidget() {
  const card = $("#adsgram-task-card");
  const el = $("#adsgram-task-el");
  if (!card || !el) return;

  const rewardSlot = card.querySelector(".ag-task-reward");
  if (rewardSlot) rewardSlot.textContent = `+${getAdRewardDisplay(ADSGRAM_TASK_CONFIG.reward)}`;

  el.addEventListener("reward", () => {
    if (adsgramTaskRewarded) return;
    adsgramTaskRewarded = true;
    const reward = getAdRewardWithBoost(ADSGRAM_TASK_CONFIG.reward);
    addCoins(reward);
    appState.adsWatched = (appState.adsWatched || 0) + 1;
    incrementAdsWatchedToday();
    refreshLevelFromXp();
    ...
    logAdTransaction("adsgram_task", null, reward);
    queueUserSync();
  });

  el.addEventListener("onError", (e) => {
    console.log("Adsgram task error:", e.detail);
  });
